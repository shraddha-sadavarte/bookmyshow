...
const SeatLayout = () => {
      ...
      <div className='flex flex-wrap items-center justify-center gap-2'>
        {Array.from({ length: count} , (_,i) => {
          ...
          return (
            ...
            border-red-500/60 cursor-pointer ${selectedSeats.includes(seatId) && "bg-red-500 text-white "}`}>
              {seatId}
            </button>
          )
        })}
  ...
  return show ? (
    ...
    md:pt-50'>
      {/* Available Timings */}
      <div className='w-60 bg-red-500/10 border border-red-500/20 rounded-lg
        ...
        <div className='mt-5 space-y-1'>
          {show.dateTime[date].map((item)=>(
            <div key={item.time} onClick={()=> setSelectedTime(item)} className={`flex items-center gap-2 px-6 py-2 w-max rounded-r-md
              cursor-pointer transition ${selectedTime?.time === item.time ? "bg-red-500 text-white" : 
              "hover:bg-red-500/20"}`}>
