...
const AddShows = () => {
  ...
  const handleDateTimeAdd = () => {
    console.log("Button clicked", dateTimeInput); 
      ...
      <div className='overflow-x-auto pb-4'>
        <div className='group flex flex-wrap gap-4 mt-4 w-max'>
          {nowPlaying.map((movie) => (
            <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:translate-y-1 transition
              duration-300`} onClick={() => setSelectedMovies(movie.id)}>
                <div className='relative rounded-lg overflow-hidden'>
                  ...
                  <div className='text-sm flex items-ceter justify-between p-2 bg-black/70 w-full absolute bottom-0 left-0'>
      ...
      <div className='mt-8'>
       ...
       <div className='mt-6'>
        ...
        <div className='inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg'>
          ...
          <button type='button' onClick={handleDateTimeAdd} className='bg-red-500/80 text-white px-3 py-2 text-sm rounded-lg hover:bg-red-500 cursor-pointer'>Add Time</button>
        </div>
       </div>
    </>
  ) : <p>Loading...</p>
